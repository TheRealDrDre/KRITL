...
for (p in unique(l3$participant)) {
  for (s in unique(l3$Structure)) {
    sub <- subset(l3, l3$participant == p & l3$Structure == s)
    val <- quantile(sub$encoding.rt, c(0.75)) + 1.5 * IQR(sub$encoding.rt)
    #    print(dim(sub))
    #val <- quantile(sub$encoding.rt, c(0.95))
    #val <- mean(sub$encoding.rt) + 3 * sd(sub$encoding.rt)  
    
    l3$encoding.rt[l3$participant == p & l3$Structure == s &
                     l3$encoding.rt > val] <- NA
    
    #val <- quantile(sub$encoding.rt, c(0.25)) - 1.5 * IQR(sub$encoding.rt)
    #val <- quantile(sub$encoding.rt, c(0.0))
    #val <- mean(sub$encoding.rt) + 3 * sd(sub$encoding.rt)  
...
for (s in unique(l4$Structure)) {
  sub <- subset(l4, l4$Structure == s)
  val <- quantile(sub$encoding.rt, c(0.75)) + 1.5 * IQR(sub$encoding.rt)
  #val <- quantile(sub$encoding.rt, c(0.95))
  #val <- mean(sub$encoding.rt) + 3 * sd(sub$encoding.rt)  
  l4$encoding.rt[l4$Structure == s &
                   l4$encoding.rt > val] <- NA
...
for (p in unique(l4$participant)) {
  for (s in unique(l4$Structure)) {
    sub <- subset(l4, l4$participant == p & l4$Structure == s)
    val <- quantile(sub$encoding.rt, c(0.75)) + 1.5 * IQR(sub$encoding.rt)
#    print(dim(sub))
    #val <- quantile(sub$encoding.rt, c(0.95))
    #val <- mean(sub$encoding.rt) + 3 * sd(sub$encoding.rt)  
    
    l4$encoding.rt[l4$participant == p & l4$Structure == s &
                   l4$encoding.rt > val] <- NA
